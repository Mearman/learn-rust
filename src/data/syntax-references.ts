...
using Point = std::pair<int, int>;`, explanation: "struct and class define types (default visibility differs). enum class is scoped. using creates type aliases." },
    ...
    // Error handling
    // =========================================================================
    { id: "err-rust", languageId: "rust", topic: "Error handling", title: "Result and Option with ? operator", code: `use std::fs;
use std::io;
...
}`, explanation: "Result<T, E> represents success or error. Option<T> represents presence or absence. The ? operator propagates errors early." },
    { id: "err-python", languageId: "python", topic: "Error handling", title: "try/except with custom exceptions", code: `class AppError(Exception):
    pass
...
def read_username() -> str:
    try:
        ...
        raise AppError(f"read failed: {e}") from e`, explanation: "Exceptions are raised and caught with try/except. Custom exceptions extend Exception. The raise...from chain preserves context." },
    { id: "err-typescript", languageId: "typescript", topic: "Error handling", title: "try/catch and Result-like patterns", code: `function readUsername(): Result<string, Error> {
    try {
...
}`, explanation: "TypeScript has try/catch for exceptions. Some codebases use Result-style discriminated unions for explicit error handling without throwing." },
    { id: "err-java", languageId: "java", topic: "Error handling", title: "try/catch with checked exceptions", code: `String readUsername() throws IOException {
...
}`, explanation: "Checked exceptions must be declared or caught. Unchecked exceptions (RuntimeException) do not. finally guarantees cleanup." },
    { id: "err-kotlin", languageId: "kotlin", topic: "Error handling", title: "try as expression with Result", code: `fun readUsername(): Result<String> = runCatching {
...
fun findUser(id: Int): String? =
    if (id == 1) "Alice" else null`, explanation: "try is an expression that returns a value. Result<T> wraps success or failure. Nullable types (T?) replace checked exceptions for simple absence." },
    { id: "err-go", languageId: "go", topic: "Error handling", title: "Explicit error return values", code: `func readUsername() (string, error) {
...
if err != nil {
    log.Fatal(err)
}`, explanation: "Errors are returned as the last value. Callers check err != nil. fmt.Errorf with %w wraps errors. No exceptions are used." },
    { id: "err-csharp", languageId: "csharp", topic: "Error handling", title: "try/catch and Result patterns", code: `string ReadUsername() {
    try {
...
string? FindUser(int id) =>
    id == 1 ? "Alice" : null;`, explanation: "Exceptions are thrown and caught with try/catch. Nullable reference types (C# 8+) track absence. finally guarantees cleanup." },
    { id: "err-cpp", languageId: "cpp", topic: "Error handling", title: "Exceptions and std::optional/std::expected", code: `std::string read_username() {
    try {
...
// C++17: std::optional<T>
std::optional<std::string> lookup(int id);`, explanation: "Exceptions are thrown and caught. std::optional (C++17) represents absence. std::expected (C++23) represents success or error without exceptions." },

    // =========================================================================
    // Struct/class definition
    // =========================================================================
    { id: "struct-rust", languageId: "rust", topic: "Struct/class definition", title: "Named and tuple structs with impl", code: `struct User {
    name: String,
    age: u32,
}
...
struct Point(f64, f64);  // tuple struct`, explanation: "Structs hold named fields. Methods are added in impl blocks. Tuple structs have positional fields. No inheritance — composition only." },
    { id: "struct-python", languageId: "python", topic: "Struct/class definition", title: "Classes with dataclass", code: `from dataclasses import dataclass
...
class Point:
    def __init__(self, x: float, y: float):
        self.x = x
        self.y = y`, explanation: "dataclass generates __init__, __repr__, and __eq__. Plain classes use __init__ for setup. Methods are defined in the class body." },
    { id: "struct-typescript", languageId: "typescript", topic: "Struct/class definition", title: "Classes and plain objects", code: `interface User {
...
}`, explanation: "Plain objects with interfaces are common for data. Classes add methods and encapsulation. Parameter properties reduce boilerplate." },
    { id: "struct-java", languageId: "java", topic: "Struct/class definition", title: "Records and traditional classes", code: `record User(String name, int age) {
    String greet() {
        return "Hello, " + name;
    }
}
...
}`, explanation: "Records (Java 16+) provide immutable data carriers with auto-generated accessors, equals, and hashCode. Traditional classes offer full encapsulation." },
    { id: "struct-kotlin", languageId: "kotlin", topic: "Struct/class definition", title: "data class with methods", code: `data class User(val name: String, val age: Int) {
    fun greet(): String = "Hello, \${'$'}name"
}

// or a plain class
class Point(val x: Double, val y: Double)`, explanation: "data class generates equals, hashCode, toString, and copy. Primary constructor parameters become properties. Plain classes are lighter." },
    { id: "struct-go", languageId: "go", topic: "Struct/class definition", title: "Structs with methods", code: `type User struct {
    Name string
    Age  int
}
...
func (u *User) Birthday() {
    u.Age++
}`, explanation: "Structs hold fields. Methods are attached via receiver arguments. Pointer receivers allow mutation. No classes or inheritance." },
    { id: "struct-csharp", languageId: "csharp", topic: "Struct/class definition", title: "Records, classes, and structs", code: `record User(string Name, int Age) {
...
}`, explanation: "Records (C# 9+) provide value-based equality. Classes are reference types. Structs are value types. init setters allow object initialiser assignment." },
    { id: "struct-cpp", languageId: "cpp", topic: "Struct/class definition", title: "Structs and classes with methods", code: `struct User {
    std::string name;
    unsigned int age;
...
};`, explanation: "struct and class differ only in default visibility (public vs private). Methods are defined inline or out-of-line. const methods cannot mutate." },

    // =========================================================================
    // Enum/variant definition
    // =========================================================================
    { id: "enum-rust", languageId: "rust", topic: "Enum/variant definition", title: "Enums carrying data", code: `enum Shape {
    Circle { radius: f64 },
...
}`, explanation: "Enums can carry named or positional data. Each variant is a distinct type within the enum. match provides exhaustiveness checking." },
    { id: "enum-python", languageId: "python", topic: "Enum/variant definition", title: "Enums and tagged unions via classes", code: `from enum import Enum, auto
...
match shape:
    ...
    case Rectangle(w=w, h=h):
        print(f"rect: {w}x{h}")`, explanation: "Standard enums hold simple values. Tagged unions are modelled with class hierarchies and matched with structural pattern matching (Python 3.10+)." },
    { id: "enum-typescript", languageId: "typescript", topic: "Enum/variant definition", title: "Union types and discriminated unions", code: `// Union of string literals
type Status = "active" | "inactive";
...
}`, explanation: "Discriminated unions use a shared literal field (kind) for narrowing. TypeScript checks exhaustiveness in switch statements. String unions replace simple enums." },
    { id: "enum-java", languageId: "java", topic: "Enum/variant definition", title: "Enums with fields and sealed interfaces", code: `enum Status { ACTIVE, INACTIVE }
...
record Rectangle(double width, double height) implements Shape {}`, explanation: "Enums can carry fields and methods. Sealed interfaces (Java 17+) create closed hierarchies that the compiler checks exhaustively." },
    { id: "enum-kotlin", languageId: "kotlin", topic: "Enum/variant definition", title: "Enum classes and sealed hierarchies", code: `enum class Status { ACTIVE, INACTIVE }
...
}`, explanation: "Enum classes hold simple constants. Sealed classes define closed hierarchies where the compiler knows all subtypes. when is exhaustive over sealed types." },
    { id: "enum-go", languageId: "go", topic: "Enum/variant definition", title: "iota constants and interface-based sum types", code: `type Status int
...
}`, explanation: "iota generates sequential constants for enum-like types. Sum types are modelled with interfaces and type switches, but exhaustiveness is not checked by the compiler." },
    { id: "enum-csharp", languageId: "csharp", topic: "Enum/variant definition", title: "Enums and discriminated unions via OneOf", code: `enum Status { Active, Inactive }
...
};`, explanation: "Standard enums hold named integers. Discriminated unions can be modelled with sealed abstract records and pattern matching. The compiler checks exhaustiveness." },
    { id: "enum-cpp", languageId: "cpp", topic: "Enum/variant definition", title: "Scoped enums and std::variant", code: `enum class Status { Active, Inactive };
...
}`, explanation: "enum class is scoped and type-safe. std::variant (C++17) provides tagged unions with std::visit for pattern matching. No compiler exhaustiveness check." },

    // =========================================================================
    // Pattern matching
    // =========================================================================
    { id: "pattern-rust", languageId: "rust", topic: "Pattern matching", title: "match with guards and destructuring", code: `fn classify(value: i32) -> &'static str {
    match value {
        0 => "zero",
        n if n > 0 => "positive",
        _ => "negative",
...
match slice {
    ...
    [] => println!("empty"),
}`, explanation: "match checks exhaustiveness. Guards (if) add conditions. .. ignores remaining fields. Slice patterns match start and end elements." },
    { id: "pattern-python", languageId: "python", topic: "Pattern matching", title: "match with structural patterns (3.10+)", code: `def classify(value: int) -> str:
    match value:
        case 0:
            return "zero"
        case n if n > 0:
...
def handle(event):
    match event:
        ...
        case {"type": "key", "code": code}:
            print(f"key: {code}")
        case _:
            print("unknown")`, explanation: "match (Python 3.10+) supports literal, sequence, mapping, and class patterns. Guards use if. _ is the wildcard." },
    { id: "pattern-typescript", languageId: "typescript", topic: "Pattern matching", title: "switch with narrowing and destructuring", code: `function classify(value: number): string {
    switch (true) {
...
// Tuple destructuring
const [first, ...rest] = [1, 2, 3];`, explanation: "switch with discriminated unions provides narrowing. Object and array destructuring work outside switch. No exhaustiveness guarantee without never checks." },
    { id: "pattern-java", languageId: "java", topic: "Pattern matching", title: "switch patterns (Java 21+)", code: `String classify(int value) {
    return switch (value) {
        case 0 -> "zero";
...
}`, explanation: "switch expressions with pattern matching (Java 21+) support type patterns, record destructuring, and guards (when). instanceof narrows in one step." },
    { id: "pattern-kotlin", languageId: "kotlin", topic: "Pattern matching", title: "when with smart casts", code: `fun classify(value: Int): String = when (value) {
    0 -> "zero"
...
val (name, age) = user
val (first, rest) = list`, explanation: "when supports values, ranges, and type checks. is triggers smart casts so members are accessible without casting. Destructuring works with data classes." },
    { id: "pattern-go", languageId: "go", topic: "Pattern matching", title: "Type switch and simple matching", code: `func classify(value int) string {
    switch {
    case value == 0:
        return "zero"
    case value > 0:
...
}`, explanation: "Switch without a condition acts like if/else chains. Type switches (v.(type)) narrow interface types. No destructuring or pattern guards." },
    { id: "pattern-csharp", languageId: "csharp", topic: "Pattern matching", title: "Switch expressions with patterns", code: `string Classify(int value) => value switch {
    0 => "zero",
    > 0 => "positive",
    _ => "negative"
};
...
}`, explanation: "switch expressions support relational, property, and positional patterns. is narrows types in one step. The compiler checks exhaustiveness." },
    { id: "pattern-cpp", languageId: "cpp", topic: "Pattern matching", title: "std::visit and structured bindings", code: `std::string classify(int value) {
...
}, variant);`, explanation: "C++ has no built-in pattern matching. Structured bindings (C++17) destructure tuples and pairs. std::visit dispatches on variant types." },

    // =========================================================================
    // Generics and type parameters
    // =========================================================================
    { id: "generic-rust", languageId: "rust", topic: "Generics and type parameters", title: "Generic functions and traits", code: `fn largest<T: PartialOrd>(list: &[T]) -> &T {
...
{
    // ...
}`, explanation: "Type parameters are declared in angle brackets. Trait bounds constrain what types can do. Where clauses keep signatures clean for complex bounds." },
    { id: "generic-python", languageId: "python", topic: "Generics and type parameters", title: "Type variables and generic functions", code: `from typing import TypeVar, Generic
...
def first[T](items: list[T]) -> T:
    return items[0]`, explanation: "TypeVar defines type parameters. Generic[T] makes classes generic. Python 3.12+ allows type parameters directly in function and class signatures." },
    { id: "generic-typescript", languageId: "typescript", topic: "Generics and type parameters", title: "Generic functions with constraints", code: `function largest<T extends { compareTo(other: T): number }>(
    list: readonly T[]
): T {
...
const w: Wrapper<string> = { value: "hello" };`, explanation: "Type parameters are declared in angle brackets. extends constrains types. Defaults reduce boilerplate. TypeScript infers types from usage." },
    { id: "generic-java", languageId: "java", topic: "Generics and type parameters", title: "Generic methods and bounded types", code: `<T extends Comparable<T>> T largest(List<T> list) {
...
}`, explanation: "Type parameters are declared in angle brackets. extends bounds types. Wildcards (? extends T) provide variance at use site." },
    { id: "generic-kotlin", languageId: "kotlin", topic: "Generics and type parameters", title: "Generic functions with reified types", code: `fun <T : Comparable<T>> largest(list: List<T>): T {
    var biggest = list[0]
...
interface Source<out T> {
    fun next(): T
}`, explanation: "Type parameters are declared in angle brackets. Reified types (inline functions) preserve generics at runtime. out/in control variance at declaration site." },
    { id: "generic-go", languageId: "go", topic: "Generics and type parameters", title: "Type parameters with constraints (Go 1.18+)", code: `type Ordered interface {
...
type Adder interface {
    int | float64 | string
}`, explanation: "Go 1.18+ supports type parameters. Constraints are interfaces with type sets. ~T includes all types with underlying type T. any is an alias for interface{}." },
    { id: "generic-csharp", languageId: "csharp", topic: "Generics and type parameters", title: "Generic methods with constraints", code: `T Largest<T>(IReadOnlyList<T> list) where T : IComparable<T> {
    var biggest = list[0];
...
}`, explanation: "Type parameters are declared in angle brackets. where constrains them. Constraints can require interfaces, a base class, a parameterless constructor, or reference/value type." },
    { id: "generic-cpp", languageId: "cpp", topic: "Generics and type parameters", title: "Templates with concepts (C++20)", code: `template <typename T>
...
struct Wrapper {
    T value;
};`, explanation: "Templates are the C++ mechanism for generics. Concepts (C++20) constrain template parameters with readable requirements. Errors are caught earlier." },

    // =========================================================================
    // Async and concurrency
    // =========================================================================
    { id: "async-rust", languageId: "rust", topic: "Async and concurrency", title: "async/await with tokio runtime", code: `use tokio;
...
use tokio::sync::mpsc;
let (tx, mut rx) = mpsc::channel(32);`, explanation: "async fn returns a Future. .await suspends until the future resolves. Tasks are spawned on a runtime (usually tokio). Channels pass messages between tasks." },
    { id: "async-python", languageId: "python", topic: "Async and concurrency", title: "async/await with asyncio", code: `import asyncio
...
asyncio.run(main())`, explanation: "async def defines coroutines. await suspends until the awaitable completes. asyncio.gather runs coroutines concurrently. asyncio.run drives the event loop." },
    { id: "async-typescript", languageId: "typescript", topic: "Async and concurrency", title: "async/await with Promises", code: `async function fetchData(url: string): Promise<string> {
...
const p = new Promise<string>((resolve, reject) => {
    // ...
    resolve("result");
});`, explanation: "async functions return Promises. await unwraps them. Promise.all runs operations concurrently. The event loop handles scheduling." },
    { id: "async-java", languageId: "java", topic: "Async and concurrency", title: "CompletableFuture and virtual threads", code: `// CompletableFuture (async pipeline)
...
}`, explanation: "CompletableFuture chains async operations. Virtual threads (Java 21+) provide lightweight concurrency with blocking-style code. Structured concurrency is incubating." },
    { id: "async-kotlin", languageId: "kotlin", topic: "Async and concurrency", title: "Coroutines with structured concurrency", code: `import kotlinx.coroutines.*
...
}`, explanation: "suspend marks async functions. Coroutines are launched in scopes for structured concurrency. async returns a Deferred. Flow represents cold async streams." },
    { id: "async-go", languageId: "go", topic: "Async and concurrency", title: "Goroutines and channels", code: `func fetchData(url string) string {
    resp, _ := http.Get(url)
...
func main() {
    ...
    wg.Wait()
}`, explanation: "go launches a goroutine. Channels communicate between goroutines. sync.WaitGroup waits for a group of goroutines to finish. No async/await keyword." },
    { id: "async-csharp", languageId: "csharp", topic: "Async and concurrency", title: "async/await with Tasks", code: `async Task<string> FetchData(string url) {
...
var result = await task;`, explanation: "async methods return Task or Task<T>. await unwraps the result. Task.WhenAll runs operations concurrently. Task.Run offloads CPU-bound work to the thread pool." },
    { id: "async-cpp", languageId: "cpp", topic: "Async and concurrency", title: "std::async, threads, and coroutines", code: `// std::async (C++11)
...
auto f = p.get_future();
p.set_value("result");`, explanation: "std::async launches asynchronous tasks. std::thread provides OS-level threads. C++20 coroutines use co_await/co_return. promise/future communicate between threads." },
] as const;
