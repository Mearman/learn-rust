...
function findLanguageConcept(languageId: string, conceptId: string): LanguageConcept | undefined {
    return LANGUAGE_CONCEPTS.find((lc) => lc.languageId === languageId && lc.conceptId === conceptId);
}
...
export function ComparisonView({ profile, active, onSelect, onOpenLesson }: ComparisonViewProps) {
        ...
        <div className={learnGrid}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "0.375rem" }}>
                    {CONCEPTS.map((c) => {
                        ...
                        return (
                            ...
                            >
                                <ArrowLeftRight size={16} style={{ color: on ? vars.colour.accentSoft : vars.colour.faint, flexShrink: 0 }} />
            ...
            <article style={{ minWidth: 0, display: "flex", flexDirection: "column", gap: "1.5rem" }}>
                ...
                <div className={noteBlock}>
                    <span>
                        <ArrowLeftRight size={14} style={{ verticalAlign: "middle", marginRight: "0.25rem" }} />
                        Comparing {languageNameForId(TARGET_LANGUAGE_ID)} with {visibleLanguages.filter((l) => l.id !== TARGET_LANGUAGE_ID).map((l) => l.name).join(", ")}.
                            ...
                            >
                                ...
                                {entry === undefined ? (
                                    ...
                                    <>
                                        ...
                                        <CodeBlock code={entry.code} label={`${entry.id}.${fileExtensionForLanguage(lang.id)}`} />
                ...
                {concept.lessonIds.length > 0 ? (
                    <div className={cheatCard}>
                        <h3 className={cheatTitle}>
                            <BookOpen size={14} style={{ verticalAlign: "middle", marginRight: "0.25rem" }} />
                            Related lessons
                        </h3>
