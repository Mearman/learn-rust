...
export function ProgressionView({ profile, onOpenLesson, onOpenConcept }: ProgressionViewProps) {
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
            <header style={{ display: "flex", flexDirection: "column", gap: "0.375rem" }}>
                ...
                <p className={lessonTagline}>
                    Concepts build on each other. Earlier layers are prerequisites for later ones.
                </p>
            </header>

            <div className={noteBlock}>
                <span>
                    <GitBranch size={14} style={{ verticalAlign: "middle", marginRight: "0.25rem" }} />
                    Top-to-bottom order. Items on the same row can be learnt in any order.
                </span>
            </div>
            ...
            {layers.map((layer, layerIndex) => (
                <div key={layerIndex}>
                    <div style={{
                        ...
                        <span style={{ color: vars.colour.faint, fontSize: "0.8rem" }}>
                            {layerIndex === 0 ? "Start here" : `Layer ${layerIndex + 1}`}
                        </span>
                    </div>
                        ...
                        {layer.map((conceptId) => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <p style={{ margin: "0 0 0.75rem", fontSize: "0.85rem", lineHeight: 1.5, color: vars.colour.dim }}>
                                        {concept.description}
                                    </p>
                                    {deps.length > 0 ? (
                                        <div style={{ marginBottom: "0.5rem", fontSize: "0.8rem", color: vars.colour.faint }}>
                                            Requires: {deps.map((d) => {
                                                const dep = CONCEPTS.find((c) => c.id === d);
                                                return dep?.title ?? d;
                                            }).join(", ")}
                                        </div>
                                    ) : null}
                                    ...
                                    <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap" }}>
                                        <button
                                            ...
                                            className={navButton}
                                            style={{ width: "auto", padding: "0.4rem 0.65rem", fontSize: "0.8rem" }}
                                        >
                                            Compare languages
                                        </button>
